...
impl<'ctx> CompilationUnit<'ctx> {
  ...
  pub fn check(&mut self) -> CheckResult {
    ...
    CheckResult {
      hir,
      used_externals: decl.used_externals, 
      symbol_table: inference.sym_table,
    }
  }
  ...
  pub fn compile(&mut self) -> Result<PathBuf> {
    ...
    run_codegen(res.hir, res.used_externals, &mut res.symbol_table, self.context.symbols(), self.context.sources(), order, self.info.name.clone(), self.info.root.clone());
