...
impl Parser {
  ...
  fn parse_attribute(&mut self) -> ParseResult<Attribute> {
    ...
    let args = if self.eat(TokenType::LeftParen) { Some(self.parse_attr_args()?) } else { None };

    self.expect(TokenType::RightBracket, "to close attribute")?;

    Ok(Attribute { id: NodeId::new(), path, args, span: self.span_from(start) })
  }
  ...
  fn parse_attr_path(&mut self) -> ParseResult<AttrPath> {
    ...
    Ok(AttrPath { id: NodeId::new(), segments, span: self.span_from(start) })
  }
  ...
  pub fn parse_identifier(&mut self) -> ParseResult<Ident> {
    ...
    match &token.token_type {
      TokenType::Identifier(name) => Ok(Ident::new(name.as_str(), token.span)),
