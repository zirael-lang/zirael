...
use zirael_type_checker::{
  GenericSymbol, GenericSymbolKind, MonoSymbolTable, MonomorphizedSymbol, MonomorphizedSymbolKind,
};
...
impl<'table, 'sources> ManglingTable<'table, 'sources> {
  ...
  pub fn get_base_name(&self, sym: &GenericSymbol) -> String {
    ...
    buff.push_str(&sym.name().to_string());
  ...
  pub fn mangle_mono(&mut self, variants: Vec<MonomorphizationId>) {
    for variant in variants {
      let mono = self.sym_table.get_monomorphized_symbol(variant).unwrap().clone();
      let generic = self.sym_table.get_generic_symbol(mono.original_symbol_id()).unwrap().clone();

      // mangle parent
      self.mangle_generic(mono.original_symbol_id());
